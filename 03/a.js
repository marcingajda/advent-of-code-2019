const {range, intersectionBy} = require('lodash');
...
const addPath = (index, x, y) => {
  !history[index] && (history[index] = []);
  history[index].push({x, y, distance: Math.abs(x) + Math.abs(y)});
};

const stepByStep = (x, y, horizontal, vertical, index) => {
  ...
  range(finalX, x).forEach((position) => addPath(index, position, y));
  range(finalY, y).forEach((position) => addPath(index, x, position));

  return {x: finalX, y: finalY}
};

const navigate = ({x, y}, part, index) => {
  const direction = part[0];
  const length = Number(part.substr(1));

  if (direction === 'L') {
    return stepByStep(x, y, -length, 0, index);
  }
  ...
  if (direction === 'U') {
    return stepByStep(x, y, 0, length, index);
  }
...
const generateWire = (wire, index) => {
  wire.split(',').reduce((currentPosition, part) => {
    return navigate(currentPosition, part, index);
  }, {x: 0, y: 0});
};
...
const intersections = intersectionBy(history[0], history[1], JSON.stringify);
